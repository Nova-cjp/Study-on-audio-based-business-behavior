...
if __name__ == "__main__":
    # wave_file_dir=r'D:\Graduation_project\audio\voice_change\wave_file'
    # wave_file_vad=r'D:\Graduation_project\audio\voice_change\wave_file_vad'
    # for i in range(len(os.listdir(wave_file_dir))):
    #     filename='section'+str(i)+'.wav'
    #     vad_filename = filename.split(".")[0] + "_vad.wav"
    #     input_file=os.path.join(wave_file_dir,filename)
    #     output_file=os.path.join(wave_file_vad,vad_filename)
    #     # 端点检测
    #     vad = VAD(input_file, min_interval=15, pt=False)
    #     sf.write(output_file, vad.wav_dat_utterance, 16000, "PCM_16")
    #     print("finish one")

    import os
    wave_file_vad = r'D:\Graduation_project\audio\voice_change\wave_file_vad'
    features_vec=[]
    for i in range(len(os.listdir(wave_file_vad))):
        filename='section'+str(i)+'_vad.wav'
        vad_file_dir=os.path.join(wave_file_vad,filename)
        features_vec.append(my_acoustic_features(vad_file_dir))
    print(features_vec)
    ...
    # wave_file = os.path.join(current_path, "audios/section7.wav")#001201

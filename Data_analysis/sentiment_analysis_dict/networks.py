...
if __name__ =='__main__':
    ...
    for dir in dirs:
        texts_dir=work_dir+'\\'+dir+"\\"+"text"
        txt_files=os.listdir(texts_dir)
        count_of_section=len(txt_files)
        record_file_dir=work_dir+"\\"+dir+"\\"+"record.txt"
        for i in range(count_of_section):
            section_file_name="section"+str(i)+".txt"
            absolute_section_file=os.path.join(texts_dir,section_file_name)
            with open(absolute_section_file, "r") as f:  # 打开文件
                data = f.read()  # 读取文件
                try:
                    result=eval(data)["result"][0]
                except Exception:
                    print(absolute_section_file)
                    with open(record_file_dir, "a")as f2:
                        f2.write('None\n')
                    with open("./error_catch.txt","a")as f3:
                        f3.write(absolute_section_file)
                        f3.write('\n')
                    continue
                with open(record_file_dir,"a")as f2:
                    f2.write(str(sa.normalization_score(data)))
                    f2.write('\n')
